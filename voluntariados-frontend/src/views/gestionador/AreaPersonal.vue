        ...
        <div class="row">
          <div class="col-lg-8 mx-auto text-center">
            ...
            <p class="hero-subtitle">
              Aquí podrás gestionar los voluntariados y realizar seguimiento de turnos y
              asistencias.
            </p>
          </div>
        </div>
    ...
    <div class="container my-5 voluntariados-table-container">
      ...
      <div class="table-with-arrow">
        <div class="pipeline-arrow-left">
          <div
            class="arrow-line"
            style="background: linear-gradient(180deg, #6c757d 0%, #0d6efd 100%)"
          ></div>
          <div class="arrow-head">
            ...
            <template #cell-inscriptos_count="{ item }">
              <div class="d-flex align-items-center justify-content-center">
                ...
                <span class="badge bg-primary">{{
                  item.inscriptos_count ?? item.voluntarios_count ?? 0
                }}</span>
              </div>
            </template>
          </AdminTable>
      ...
      <div class="table-with-arrow">
        <div class="pipeline-arrow-left">
          <div
            class="arrow-line"
            style="background: linear-gradient(180deg, #0d6efd 0%, #ffc107 100%)"
          ></div>
          <div class="arrow-head">
          ...
          >
            ...
            <template #cell-inscriptos_pendientes="{ item }">
              <div class="d-flex align-items-center justify-content-center">
                ...
                <span class="badge bg-warning text-dark">{{
                  inscriptosPendientesMap[item.id] ?? 0
                }}</span>
              </div>
            </template>
            <template #actions="{ item }">
              <button
                ...
                @click.stop="inscripcionesmanagement(item)"
              >
      ...
      <div class="table-with-arrow">
        <div class="pipeline-arrow-left">
          <div
            class="arrow-line"
            style="background: linear-gradient(180deg, #ffc107 0%, #198754 100%)"
          ></div>
          <div class="arrow-head">
            ...
            <template #cell-incomplete_turnos="{ item }">
              <div class="d-flex align-items-center justify-content-center">
                <i
                  ...
                  v-if="(incompleteTurnosMap[item.id] ?? 0) > 0"
                ></i>
                <span
                  :class="[
                    'badge',
                    (incompleteTurnosMap[item.id] ?? 0) > 0
            ...
            <template #cell-inscriptos_count="{ item }">
              <div class="d-flex align-items-center justify-content-center">
                ...
                <span class="badge bg-success">
                  {{ inscriptosAceptadosMap[item.id] ?? 0 }}/{{
                    inscriptosTotalesMap[item.id] ?? 0
                  }}
                </span>
              </div>
            </template>
            ...
            <template #cell-nombre="{ item }">
              <div class="d-flex align-items-center">
                ...
                <div>
                  <span class="fw-bold">{{ item.nombre }}</span>
                  <span
                    v-if="asistenciaCompletaMap[item.id] === false"
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      voluntariadosProximos: [] as Array<{
        id: number;
        nombre: string;
      ...
      voluntariadosConvocatoria: [] as Array<{
        id: number;
        nombre: string;
        fecha_inicio_convocatoria?: string | null;
        fecha_fin_convocatoria?: string | null;
      ...
      voluntariadosPreparacion: [] as Array<{
        id: number;
        nombre: string;
        fecha_inicio_convocatoria?: string | null;
        fecha_fin_convocatoria?: string | null;
      ...
      voluntariadosFinalizados: [] as Array<{
        id: number;
        nombre: string;
        fecha_inicio_cursado?: string | null;
        fecha_fin_cursado?: string | null;
  ...
  methods: {
    async loadUpcomingData() {
      ...
      try {
        ...
        this.errorUpcoming =
          err?.response?.data?.detail || "Error al cargar los voluntariados próximos";
      } finally {
    ...
    async loadConvocatoriaData() {
      ...
      try {
        ...
        this.errorConvocatoria =
          err?.response?.data?.detail || "Error al cargar los voluntariados en convocatoria";
        this.voluntariadosConvocatoria = [];
      } finally {
    ...
    async loadPreparacionData() {
      ...
      try {
        ...
        this.errorPreparacion =
          err?.response?.data?.detail || "Error al cargar los voluntariados en preparación";
        this.voluntariadosPreparacion = [];
      } finally {
    ...
    async loadFinishedData() {
      ...
      try {
        ...
        this.errorFinished =
          err?.response?.data?.detail || "Error al cargar los voluntariados finalizados";
      } finally {
    ...
    async loadHorasRealizadasForVoluntariados(items: Array<{ id: number }>) {
      const requests = items.map(async (v) => {
        try {
          ...
          const turnos =
            turnosResp.data && turnosResp.data.results ? turnosResp.data.results : turnosResp.data;
    ...
    async loadTurnosCountForVoluntariados(items: Array<{ id: number }>) {
      ...
      const results = await Promise.all(requests);
      results.forEach((r) => {
        this.turnosCountMap[r.id] = r.count;
      });
    },
    async loadIncompleteTurnosForVoluntariados(items: Array<{ id: number }>) {
      ...
      const results = await Promise.all(requests);
      results.forEach((r) => {
        this.incompleteTurnosMap[r.id] = r.count;
      });
    },
    async loadInscriptosPendientesForVoluntariados(items: Array<{ id: number }>) {
          ...
          .then((resp: any) => {
            ...
            const pending = Array.isArray(arr)
              ? arr.filter((insc: any) => {
                  ...
                  const matchesVoluntariado =
                    insc.voluntariado === v.id || insc.voluntariado?.id === v.id;
      ...
      const results = await Promise.all(requests);
      results.forEach((r) => {
        this.inscriptosPendientesMap[r.id] = r.count;
      });
    },
    async loadInscriptosTotalesForVoluntariados(items: Array<{ id: number }>) {
      ...
      const results = await Promise.all(requests);
      results.forEach((r) => {
        this.inscriptosTotalesMap[r.id] = r.count;
      });
    },
    async loadInscriptosAceptadosForVoluntariados(items: Array<{ id: number }>) {
      ...
      const results = await Promise.all(requests);
      results.forEach((r) => {
        this.inscriptosAceptadosMap[r.id] = r.count;
      });
    },
    async loadData() {
      this.loading = true;
    ...
    async loadAsistenciaCompletaForVoluntariados(items: Array<{ id: number }>) {
      ...
      const results = await Promise.all(requests);
      results.forEach((r) => {
        this.asistenciaCompletaMap[r.id] = r.completa;
      });
    },
