...
<!-- src/components/admin/AdministradorModal.vue -->
<template>
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">
            ...
            {{ isEdit ? 'Editar' : 'Crear' }} Administrador
          </h5>
...
export default defineComponent({
  name: 'AdministradorModal',
  props: {
    show: { type: Boolean, required: true },
    isEdit: { type: Boolean, default: false },
    administradorData: { type: Object as PropType<AdministradorFormData>, required: true }
  },
  emits: ['close', 'save'],
  data() {
    return {
  ...
  watch: {
    ...
    administradorData: { immediate: true, deep: true, handler(newData) { if (newData) { this.formData = { ...newData } } } }
  },
  methods: {
    async loadOptions() {
      try {
    ...
    resetForm() {
      ...
      this.saving = false
      if (!this.isEdit) {
        this.formData = { id: null, nombre: '', apellido: '', dni: '', fecha_nacimiento: '', telefono: '', email: '', direccion: '', localidad: null }
      }
    },
    async handleSubmit() {
